...
struct Hosted {
    ...
    /// same ring in a tick — `advance` polls after its depth sync, `Ring::begin`
    /// polls under backpressure, `set_depth` polls — and each discards its
    /// result. Whichever of them retires the frame, the outer poll then sees
    /// nothing left, reports `false`, and the retired frame is never handed to
    /// the board: a scene that renders in a burst and goes quiet keeps
    /// compositing the previous generation for as long as it stays quiet. A
    /// generation cannot be stolen — it either advanced since the last publish
    /// or it did not.
    ///
    /// Reset to 0 wherever the ring is REBUILT (`resize` calls `Ring::replace`,
    /// which restarts the ring's own generation at 0) — that collision is the
    /// reason the earlier code tracked a per-call signal instead.
    published_gen: u64,
    ...
    while let Ok(first) = rx.recv() {
        ...
        if tick {
            ...
            for (id, h) in hosted.iter_mut() {
                ...
                // The signal is the ring's GENERATION against the one last
                // published, not whether THIS poll was the call that retired a
                // frame. The per-call signal was chosen to dodge a collision — a
                // rebuilt ring restarts at 0, so a remembered value matches the
                // new ring's first frame and skips it — but that is handled where
                // it belongs, by resetting `published_gen` in `resize`, the only
                // place the counter can go backwards. What the per-call signal
                // could not survive is theft: `advance`, `Ring::begin` and
                // `set_depth` all poll and discard, so whichever one retires the
                // frame leaves this call with nothing to report. See
                // `published_gen`.
                let _ = h.ring.poll() || moved;
                let generation = h.ring.generation();
                if generation != h.published_gen {
...
fn spawn(
    ...
    Ok(Hosted { runtime, ring, size, due_rate: None, due_cap: None, published_gen: 0, seq: 0, depth_failed: Attempt::new() })
}
...
///
/// Called when the ring's generation has moved past the one last published (see
/// `Hosted::published_gen`). The ring restarting at 0 on a rebuild is handled by
/// resetting that field in `resize`, so the comparison cannot collide across a
/// resize the way a bare remembered generation would.
