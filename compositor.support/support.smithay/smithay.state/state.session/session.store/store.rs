...
impl SessionStore {
    ...
    /// Remember a name under a session WITHOUT taking a live grant.
    ///
    /// The durable half of the store dies with the compositor, while the
    /// placeholder that carries the same identity is persisted per world. After a
    /// restart the two are out of step: `restore_toplevel` would find the session
    /// unknown and the client would be told its toplevel is new, losing exactly
    /// the state this protocol exists to restore. Seeding from the persisted
    /// placeholders puts the remembered half back before any client can ask.
    ///
    /// Only `sessions` is touched — `granted`/`added` are about what is live
    /// right now, and nothing is live at boot.
