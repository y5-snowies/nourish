...
struct Claim {
    /// `None` when the launch produced no pid. The claim is still worth keeping:
    /// the token predicate below needs no pid, and it is the one that survives
    /// the cases the pid chain cannot follow.
    pid: Option<u32>,
...
///
/// `pid` is optional on purpose. Gating registration on it made the pid a hard
/// requirement for the whole mechanism — including the TOKEN predicate, which
/// exists precisely because the pid chain dead-ends on a double-fork, a `setsid`
/// or a sandbox. In exactly those cases there was no claim at all, so the more
/// robust signal could never be tried. A claim with neither predicate is refused,
/// since nothing could ever resolve to it.
pub fn register(pid: Option<u32>, placeholder: Uuid, known_session: Option<String>, token: String) {
    if pid.is_none() && token.is_empty() {
        return;
    }
    ...
    // Upsert on the token. One launch registers twice — once on the click with
    // no pid (so a fast client can resolve immediately) and once when the
    // outcome lands carrying the pid — and those are the same claim, not two.
    // Matched on the token because that is the only key both calls share.
    let existing = claims
        .iter_mut()
        .find(|c| !c.token.is_empty() && c.token == token && c.placeholder == placeholder);
...
pub fn resolve(client_pid: i32) -> Option<String> {
    ...
    let live: Vec<(Option<u32>, String, String)> = {
    ...
    // 2. Pid tree. Only claims that HAVE a pid can answer here; a pidless one is
    //    reachable through the token alone, which is the whole reason it is kept.
    let mut pid = pid0;
    for _ in 0..MAX_DEPTH {
        if let Some((_, _, mint)) = live.iter().find(|(claimed, _, _)| *claimed == Some(pid)) {
            return Some(mint.clone());
        }
        pid = parent_of(pid)?;
        if pid <= 1 {
