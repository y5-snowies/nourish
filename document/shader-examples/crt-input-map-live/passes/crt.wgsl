// "crt-input-map" — THE SAME CURVE, SERVED FROM A BAKED MAP.
// THAT KNOWS ABOUT IT.
//
...
#import crt_live::warp::hit_inverse
...
fn fs_main(@builtin(position) frag: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // THE one displacement, at this frame's instant.
    // The clock, because this bundle's warp animates. The engine's generated grid
    // pass passes the same `pc.res_zoom_time.w`, so the map and the picture are
    // the same function evaluated at the same instant.
    let warped = hit_inverse(uv, res, vec4<f32>(curve, 0.0, 0.0, 0.0), pc.res_zoom_time.w);
