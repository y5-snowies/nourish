...
/// Resolve a selection value to a bundle directory: a compiled-in bundle to its
/// virtual root, an absolute path verbatim, anything else a folder name under
/// `background_shader_dir()`.
///
/// The embedded branch is first so a compiled-in bundle keeps working even if a
/// disk folder of the same name exists — the `builtin:` prefix says which one was
/// selected, and the two would otherwise be decided by install order.
pub fn resolve_ref(value: &str) -> PathBuf {
    if let Some(name) = compositor_pipeline_bundle_embed_base::embed::bundle_of(value) {
        return compositor_pipeline_bundle_embed_base::embed::path_of(name);
    }
