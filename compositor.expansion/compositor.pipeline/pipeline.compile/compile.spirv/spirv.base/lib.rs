...
/// Compile an already-composed naga module (e.g. from naga_oil) to a Vulkan
/// module; a fragment-only pass is paired with the fullscreen vertex.
pub fn module_to_vulkan(module: Module, id: u64) -> Result<VulkanModule, String> {
    let frag_entry = entry(&module, ShaderStage::Fragment).ok_or("composed: no fragment entry")?;
    if let Some(vert_entry) = entry(&module, ShaderStage::Vertex) {
        return Ok(VulkanModule { id, spv: to_spirv(&module)?, vert_spv: None, vert_entry, frag_entry });
    }
    let vs = naga::front::wgsl::parse_str(FULLSCREEN_VS).map_err(|e| format!("vs parse: {e:?}"))?;
    let vert_entry = entry(&vs, ShaderStage::Vertex).ok_or("vs: no vertex entry")?;
    Ok(VulkanModule { id, spv: to_spirv(&module)?, vert_spv: Some(to_spirv(&vs)?), vert_entry, frag_entry })
}
