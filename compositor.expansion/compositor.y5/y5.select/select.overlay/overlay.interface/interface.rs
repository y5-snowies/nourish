...
/// the compile-time default (`WorldAtCursor`). Read fresh each frame so a touch
/// mode change re-decides on the next create.
fn placement_mode(state: &Loop) -> Placement {
    if state.inner.touch.tool_mode == TouchMode::Select {
